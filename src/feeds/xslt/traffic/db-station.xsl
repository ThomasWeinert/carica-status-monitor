...
<xsl:stylesheet
    ...
    <xsl:template match="/*">
        ...
        <atom:feed>
            <atom:title>
                <xsl:value-of select=".//div[func:contains-token(@class, 'rline')]"/>
            </atom:title>
            ...
            <xsl:for-each select=".//div[func:contains-token(@class, 'sqdetailsDep')]">
                <atom:entry>
                    ...
                    <atom:id>urn:db/departures/<xsl:value-of select="$route"/>/<xsl:value-of select="$direction"/>
                    </atom:id>
                    <atom:updated>
                        <xsl:value-of select="$currentDate"/>
                    </atom:updated>
    ...
    <xsl:template name="image-by-route">
        ...
        <xsl:param name="type"
                   select="str:upper(substring-before(translate($route, '1234567890', '          '), ' '))"/>
